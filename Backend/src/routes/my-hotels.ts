...
router.post("/",
 ...
 try{
    ...
    const imageUrls = await uploadImages(imageFiles);// resolevs url of uploaded images
...
router.put("/:hotelId",verifyToken,upload.array("imageFiles"),async(req:Request,res:Response)=>{
    try {
        ...
        const updatedIamgeUrls=await uploadImages(files);
        hotel.imageUrls=[...updatedIamgeUrls,
